...
if __name__ == '__main__':
    ...
    for param_group in optimizer.param_groups:
        lr = param_group['lr']
    start_t = time.time()            
    lossTr, acc1_tr, acc5_tr = train_one_iter(num_gpu, trainLoader, 
    ...
    print("Accuracy(QAT) : {} Top1: {:.4f} Top5: {:.4f}".format(model_name, qat_acc1_val, qat_acc5_val))     
    print("Accuracy(Quantized) : {} Top1: {:.4f} Top5: {:.4f}".format(model_name, acc1_val, acc5_val)) 
    
    this_state_dict = model.state_dict()
    this_savedir = test_config['save_dir']
    model_file_name = this_savedir + '/quantized_' + Max_name.split('/')[2] + '.pth'
