...
def get_optimizer(optim, params_set, args):
    if optim == "SGD":
        optimizer = torch.optim.SGD(params_set, args.lr,
                                    momentum=args.momentum, weight_decay=args.weight_decay,
                                    nesterov=False)
    elif optim == "RMS":
        optimizer = torch.optim.RMSprop(params_set, args.lr,
                                        alpha=0.9, momentum=0.9, eps=1e-8,
                                        weight_decay=args.weight_decay)
    elif optim == "Adam":
        optimizer = torch.optim.Adam(params_set, args.lr,
                                     betas=(0.9, 0.999), eps=1e-08,
                                     weight_decay=args.weight_decay)
    elif optim == "AdamW":
        optimizer = torch.optim.AdamW(params_set, args.lr,
                                      betas=(0.9, 0.999), eps=1e-08,
                                      weight_decay=args.weight_decay,
                                      amsgrad=args.amsgrad)
    elif optim == "QSGD":
        optimizer = QSGD(params_set, args.lr,
                         momentum=args.momentum, weight_decay=args.weight_decay,
                         nesterov=False, clip_by=args.clip_by,
                         toss_coin=args.toss_coin, noise_decay=args.noise_decay)
    elif optim == "QRMS":
        optimizer = QRMSprop(params_set, args.lr,
                             alpha=0.9, momentum=args.momentum, eps=1e-8,
                             weight_decay=args.weight_decay,
                             clip_by=args.clip_by, toss_coin=args.toss_coin,
                             noise_decay=args.noise_decay)
    elif optim == "QAdam":
        optimizer = QAdam(params_set, args.lr,
                          betas=(0.9, 0.999), eps=1e-08,
                          weight_decay=args.weight_decay,
                          amsgrad=args.amsgrad, clip_by=args.clip_by,
                          toss_coin=args.toss_coin, noise_decay=args.noise_decay)
    elif optim == "QAdamW":
        optimizer = QAdamW(params_set, args.lr,
                           betas=(0.9, 0.999), eps=1e-08,
                           weight_decay=args.weight_decay,
                           amsgrad=args.amsgrad, clip_by=args.clip_by,
                           toss_coin=args.toss_coin, noise_decay=args.noise_decay)
...
class QSGD(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
                 ...
                 clip_by=0.0, toss_coin=True, noise_decay=1e-3):
...
class QRMSprop(Optimizer):
    ...
    def __init__(self, params, lr=1e-2, alpha=0.99, eps=1e-8, weight_decay=0, momentum=0,
                 centered=False, beta=0.9, clip_by=0.0,
                 toss_coin=True, noise_decay=1e-3):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps:
