...
class QAdamN(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, amsgrad=False, 
                 clip_by = 1e-3, toss_coin = True, 
                 noise_decay = 1e-2, nesterov=True):
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, amsgrad=amsgrad, 
                        clip_by = clip_by, toss_coin = toss_coin,
                        noise_decay = noise_decay, nesterov=nesterov)
        self.is_warmup = True
        super(QAdamN, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(QAdamN, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                nesterov = group['nesterov']
                state = self.state[p]
                norm_size = [p.size(0)] + [1] * (len(p.shape) - 1)
                # State initialization
                if len(state) == 0:
                    state['step'] = 0 
                    state['restart_step'] = 0
                    # Exponential moving average of gradient values
                    if len(p.shape) > 1:
                        state['exp_avg_scale'] = torch.zeros_like(p.data).view(p.size(0), -1).sum(dim=1).view(norm_size)
                    state['exp_avg'] = torch.zeros_like(p.data, memory_format=torch.preserve_format)
                    # Exponential moving average of squared gradient values
                    state['exp_avg_sq'] = torch.zeros_like(p.data, memory_format=torch.preserve_format)
                ...
                if len(p.shape) > 1:
                    # Calc scale gradients
                    exp_avg_scale = state['exp_avg_scale']
                    p_norm = p.data.view(p.size(0), -1).norm(dim=1).view(norm_size)
                    n_data = p.data / p_norm.add_(group['eps'])

                    scale_grad = (grad * n_data).view(p.size(0), -1).sum(dim=1).view(norm_size)
                    exp_avg_scale.mul_(beta1).add_(1 - beta1, scale_grad)
                    scale_denom = (((n_data**2) * exp_avg_sq).view(p.size(0), -1).sum(dim=1).sqrt().view(norm_size)
                                   / math.sqrt(bias_correction2)).add_(group['eps'])

                    if nesterov:
                        perturb_scale = (beta1 * exp_avg_scale + (1 - beta1) * scale_grad) / scale_denom
                    else:
                        perturb_scale = exp_avg_scale / scale_denom

                    # decouple scale perturbation
                    perturb -= n_data * (n_data * perturb).view(p.size(0), -1).sum(dim=1).view(norm_size)
                    p.data.add_(-step_size, n_data * perturb_scale)
